...
print(clean_all["LocationAbbr"].unique())
...
def expected_num_cycles(state, attribute):
    state_df = clean_all[summary_success_all["LocationAbbr"] == state]

    attribute_df = state_df[state_df["Type"] == attribute]

    result = attribute_df["Expected # of Cycles"].mean()
